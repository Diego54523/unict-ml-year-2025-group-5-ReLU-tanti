...
def stratified_split_from_indices(indices, full_targets, val_split=0.15, seed=42):
    """
    ...
    train_rel, val_rel, _ = stratified_split_indices(t, val_split=val_split, test_split=0.0, seed=seed)
...
def main():
    ...
    K = max(5, int(K_FOLDS))
    folds = stratified_kfold_indices(targets, k=K, seed=SEED, shuffle=True)
    ...
    for fold in range(K):
        ...
        train_idx, val_idx = stratified_split_from_indices(
            outer_train_idx, targets, val_split=INNER_VAL_SPLIT, seed=SEED + fold
        )
        ...
        test_dataset = SubsetWithTransform(base_dataset, outer_test_idx, transform=val_test_transform)
        ...
        test_loss, test_acc, test_balacc, _, _ = evaluate(
